package com.github.xdptdr.mbwar.jaxrs.clients.ort;
...
public class OrtCLI {
	...
	private static final String ORT = "http://localhost:8080/mbwar/rs/ort";
		...
		while (running) {
			...
			try {
				...
				if (CLI.match("quit", args, 0)) {
					...
					response = client.target(ORT + "/get").request().get();
				}
