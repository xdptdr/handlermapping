package com.github.xdptdr.mbwar.jaxrs.clients.ubi;
...
public class UbiCLI {
	...
	private static final String UBI = "http://localhost:8080/mbwar/rs/ubi";
		...
		while (running) {
			...
			try {
				...
				if (CLI.match("quit", args, 0)) {
					...
					response = client.target(UBI).request().get();
				}
