package com.github.xdptdr.mbwar.jaxrs.clients.mux;
...
public class MuxCLI {
	...
	private static final String MUX = "http://localhost:8080/mbwar/rs/mux";
		...
		while (running) {
			...
			try {
				...
				if (CLI.match("quit", args, 0)) {
					...
					response = client.target(MUX + "/get").queryParam("p", p).request().get();
				}
